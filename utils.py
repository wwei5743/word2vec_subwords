...
def generate_batches(words_int, int_to_words, batch_size, window_size):
    ...
    for index in range(math.ceil(len(words_int) / batch_size) + 1):
        batch = words_int[index * batch_size: (index + 1) * batch_size]
        input = list()
        target = list()
