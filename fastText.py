...
#     softmax_weights = tf.Variable(tf.truncated_normal(shape = (VOCAB_SIZE, EMBEDDING_SIZE), stddev=1.0 / math.sqrt(EMBEDDING_SIZE)))
#     softmax_biases = tf.Variable(tf.zeros([VOCAB_SIZE]))
...
with tf.Session(graph=train_graph) as sess:
    ...
    for epoch in range(EPOCHS):
        ...
        for subwords_input, target in batch_gen:
            feed_dict = {train_inputs: subwords_input, train_labels: np.array([target]).T}
            ...
            if iteration != 0 and iteration % 500 == 0:
                ...
                print('Average loss: {}'.format(total_loss / 500))
                print('{} sec/batch'.format((end_time - start_time) / 500))
                total_loss = 0
                start_time = time.time()
            iteration += 1
